...
export class EventEmiter{
  ...
  on(eventType, listener){
    if(this[listenerList][eventType] === undefined){
      ...
      if(this[listenerList][eventType].length === 10){
        throw new RangeError(`error add listener: the listeners of type ${eventType} is more than maximum ${this[maxListener]}`);
      }else{
  ...
  emit(eventType, ...args){
    this[listenerList][eventType] && this[listenerList][eventType].forEach(item => {
      item.call(null, ...args);
    })
  }
  removeListener(eventType, listener){
    this[listenerList][eventType] && this[listenerList][eventType].forEach((item, index, arr) => {
      if(item === listener){
        arr.splice(index, 1);
      }
    })
